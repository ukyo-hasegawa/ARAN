...
Certificate_Format Makes_Certificate(char own_ip, char own_public_key, char t, char expires) {
    Certificate_Format Certificate;
    Certificate.own_ip[0] = own_ip;
    Certificate.own_public_key[0] = own_public_key;
    Certificate.t[0] = t;
...
void serialize_forwarding_data(const RDP_format& forwarding_rdp, std::vector<uint8_t>& buf) {
    auto serialize_string = [&buf](const std::string& str) {
        std::uint32_t len = str.size();
        buf.push_back((len >> 0) & 0xFF);
        buf.push_back((len >> 8) & 0xFF);
